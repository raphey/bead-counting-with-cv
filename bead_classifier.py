...
def conv_net(x_tensor, ws, bs, dropout):
    ...
    # Output Layer - bead likelihood prediction, 0.0 to 1.0.
...
def train(data_directory, save=False):
    ...
    bead_data = import_data(data_directory + '/beads', extra_dim=True)
    non_bead_data = import_data(data_directory + '/non_beads', extra_dim=True)
    training, validation, testing = prepare_data(bead_data, non_bead_data, flat=False)
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            pos_valid_acc = sess.run(acc, feed_dict={x: validation[0], y_: validation[1]})
            neg_valid_acc = sess.run(acc, feed_dict={x: validation[2], y_: validation[3]})

            print('Epoch: {:>4}/{}   Training cost: {:<5.1f}   Pos. & neg. val. acc.: {:.4f},  {:.4f}'.format(
                  e + 1, epochs, epoch_loss, pos_valid_acc, neg_valid_acc))

        pos_test_acc = sess.run(acc, feed_dict={x: testing[0], y_: testing[1]})
        neg_test_acc = sess.run(acc, feed_dict={x: testing[2], y_: testing[3]})
...
def find_and_save_mistakes(image_directory, correct_label, save_directory):
    ...
    with tf.Session() as sess:
        ...
        for img_path in glob.glob(image_directory + '/*.png'):
            ...
            bead_likelihood = sess.run(y, feed_dict={x: gray_img})
            if not sess.run(tf.equal(tf.round(bead_likelihood), label)):
                print("--------------")
                print("Incorrectly classified image:" + img_name)
                print("Correct label:", correct_label)
                print("Calculated likelihood:", bead_likelihood[0][0])
...
if __name__ == '__main__':
    ...
    # find_and_save_mistakes('training_data/set3/beads', 1.0, 'training_data/set3/false_negatives')
    # find_and_save_mistakes('training_data/set2/non_beads', 0.0, 'training_data/set2/false_positives')
