...
import os
from bead_classifier import conv_net
import csv
import glob
...
def find_beads(image):
    """
    Given a grayscale 448x448 image, uses a trained CNN to get a list of beads.

    The process is:
    1) Pass a 9x9 window across all image locations, minus some padding, and
       for each one, find an associated probability of it being a bead.
    2) Go through the entire list of probabilities in decreasing order. Mark
       a bead as found if it has probability above a certain cutoff, it is a
       local maximum of probability, and it is a minimum distance away from
       other beads that have already been found

    Returns a list of detected beads in (x, y) form. Note that these coordinates
    ...
    with tf.Session() as sess:
        ...
        for x in range(4, 444):
            for y in range(4, 444):
                bead_img = image[y - 4: y + 5, x - 4: x + 5].astype(float)
...
def hi_res_find_beads(image_4x):
    """
    Higher resolution bead detector.
    """
    ...
    with tf.Session() as sess:
        ...
        for x in range(19, 1792 - 20):
            for y in range(19, 1792 - 20):
                ...
                bead_image = downsampled_imgs[shift_y][shift_x][scaled_y - 4: scaled_y + 5, scaled_x - 4: scaled_x + 5]

                img_list.append(bead_image)
        ...
        b_probs_lookup = np.zeros(shape=(1792, 1792))
        ...
        for x in range(1753):
            for y in range(1753):
                p = tf_model_output[x, y, 0]
                b_probs.append((p, x + 19, y + 19))
                b_probs_lookup[y + 19, x + 19] = p
        ...
        # Grab valid beads
        bs = []
        for p, x, y in b_probs:
            if p < 0.99:
                break
            if any(p < b_probs_lookup[y + dy, x + dx] for dx in (-1, 0, 1) for dy in (-1, 0, 1)):
                continue
            if any(distance(c[0], c[1], x, y) < 10 for c in bs):
                continue
            bs.append((x, y))
...
def group_beads_by_cluster(bs, d_data):
    """
    Given a list of detected beads and droplet data, returns a list of beads grouped
    by the cluster they belong to, if they belong to a valid cluster.
    The assumption is that a bead belongs to the cluster of whatever cluster circle
    ...
    bs_by_cluster = [[] for _ in range(len(d_clusters))]

    # Wiggle room for bead containment--can lead to some false positives on the edges
    edge_error = 12

    # For each bead, check if valid, and if so, link it to its containing droplet cluster
    for x, y in bs:
        ...
        if any(distance(x, y, d_x, d_y) < d_r + edge_error for d_x, d_y, d_r in d_clusters[i]):
            ...
            bs_by_cluster[i].append((x, y))

    return bs_by_cluster


def write_image_output(orig_img_4x, d_data, cluster_bs, display_image=True, show_droplets=False,
                       save_image=False, save_path='images/output.png'):
    """
    Displays and/or writes bead and droplet info to disk.
    """
...
def get_frequencies(bs_by_cluster):
    """

    """
    bead_counts = [len(bs) for bs in bs_by_cluster]
    bead_counts.sort()
    max_count = bead_counts[-1]
    fs = [0] * (max_count + 1)
    for count in bead_counts:
        fs[count] += 1
...
def analyze_image(img_path, hi_res=False, display_image=False, show_droplets=False,
    ...
    """
    Given an image path, detects droplets, detects beads, associates beads with
    droplets, saves/displays image, saves/displays frequencies (beads per droplet).
    """
    ...
    if hi_res:
        beads = hi_res_find_beads(grayscale_image_4x)
    else:
        beads = find_beads(grayscale_image)

    beads_by_cluster = group_beads_by_cluster(beads, droplet_data)

    write_image_output(original_image_4x, droplet_data, beads_by_cluster,
                       display_image=display_image, show_droplets=show_droplets,
                       save_image=save_image)

    frequencies = get_frequencies(beads_by_cluster)

    print("bead per droplet frequencies for {}:".format(image_name))
...
if __name__ == '__main__':
    ...
    analyze_image(img_path='images/test_array_1.png', display_image=False, hi_res=False, show_droplets=True, save_image=True)
