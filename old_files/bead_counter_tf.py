...
def bead_filter(img, x_c, y_c, window=0, threshold=0.8):
    """
    ...
    for x_shift in range(-window, window + 1):
        for y_shift in range(-window, window + 1):
            ...
            bead_likelihood = sess.run(y_pred, feed_dict={x_input: reshaped_img})
...
def save_bead(img, x_b, y_b, a=14):
    """
    Write the given portion of image to a new .png file with dimensions 2a by 2a.
    """
    if not (a <= x_b <= len(img[0]) - a and a <= y_b <= len(img) - a):
        return
    bead_img = img[y_b - a: y_b + a, x_b - a: x_b + a]
    if len(bead_img[0]) < 2 * a:
        print(x_b, y_b)
        print(len(bead_img), len(bead_img[0]))
        quit()
    count_label = str(100000 + bead_counter)[1:]
    cv2.imwrite('training_data/sample_{}_{}_x{}_y{}.png'.format(2 * a, count_label, x_b, y_b), bead_img)
