...
def train(save=False, verbose=False):
    ...
    bead_data = import_data('training_data/set1/beads', full_normalization=True)
    non_bead_data = import_data('training_data/set1/non_beads', full_normalization=True)
    training, validation, testing = prepare_data(bead_data, non_bead_data, flat=True)
    ...
    if save:
        np.save('classifier_data/simple_bead_classifier/weight.npy', weight)
        np.save('classifier_data/simple_bead_classifier/bias.npy', bias)
